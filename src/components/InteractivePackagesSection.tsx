...
const InteractivePackagesSectionComponent = () => {
  ...
  const handleOrderClick = () => {
    ...
    const message = `سلام! می‌خوام پلن ${gig.toLocaleString('fa-IR')} گیگابایتی برای ${months.toLocaleString('fa-IR')} ماه با پروتکل‌های (${protocolNames}) سفارش بدم.`;
    window.open(`https://t.me/ArminSQ?text=${encodeURIComponent(message)}`, '_blank');
  };
  ...
  const handleGigChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    else if (Math.abs(val - 50) <= 10) val = 50;
    else if (Math.abs(val - 100) <= 10) val = 100;
    else if (Math.abs(val - 150) <= 10) val = 150;
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center max-w-3xl mx-auto mb-12 space-y-4">
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-black text-white tracking-tight text-center">
            <BinaryText binaryClassName="text-pink-500/30" leftBinary="0101" rightBinary="1010">پلن خودت رو بساز! 💎</BinaryText>
          </h2>
          <p className="text-slate-400 text-sm sm:text-base leading-relaxed text-center">حجم دلخواهت رو انتخاب کن، پروتکل‌های مورد نظرت رو تیک بزن و قیمت نهایی رو همین الان ببین.</p>
        </div>
        ...
        <div className="rounded-3xl bg-slate-900/80 border border-white/10 p-6 sm:p-10 shadow-2xl grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="space-y-8">
            <div>
              <label htmlFor="traffic-volume" className="flex items-center justify-between mb-4 text-sm font-bold text-white">
                <span className="flex items-center gap-2"><Database className="w-4 h-4 text-emerald-400" /> حجم سرویس</span>
                <span className="text-emerald-300 font-bold text-lg">{gig.toLocaleString('fa-IR')} گیگابایت</span>
              </label>
              <input id="traffic-volume" type="range" min={10} max={200} step={1} value={gig} onChange={handleGigChange} className="w-full h-3 bg-slate-700 rounded-full appearance-none cursor-pointer transition-all duration-300 [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-6 [&::-webkit-slider-thumb]:h-6 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-emerald-400 [&::-webkit-slider-thumb]:border-4 [&::-webkit-slider-thumb]:border-slate-900 [&::-webkit-slider-thumb]:shadow-[0_0_15px_rgba(16,185,129,0.8)] [&::-moz-range-thumb]:w-6 [&::-moz-range-thumb]:h-6 [&::-moz-range-thumb]:rounded-full [&::-moz-range-thumb]:bg-emerald-400 [&::-moz-range-thumb]:border-4 [&::-moz-range-thumb]:border-slate-900" dir="rtl" aria-label="حجم سرویس" aria-valuemin={10} aria-valuemax={200} aria-valuenow={gig} aria-valuetext={`${gig} گیگابایت`} style={{ background: `linear-gradient(to left, #10b981 0%, #10b981 ${gigPercentage}%, #334155 ${gigPercentage}%, #334155 100%)` }} />
              <div className="flex justify-between text-xs text-slate-500 mt-2"><span>۱۰</span><span>۵۰</span><span>۱۰۰</span><span>۱۵۰</span><span>۲۰۰</span></div>
            </div>

            <div>
              <label htmlFor="traffic-duration" className="flex items-center justify-between mb-4 text-sm font-bold text-white">
                <span className="flex items-center gap-2"><Clock className="w-4 h-4 text-sky-400" /> مدت زمان (هر ۳۰ روز)</span>
                <span className="text-sky-300 font-bold text-lg">{months.toLocaleString('fa-IR')} ماه</span>
              </label>
              <input id="traffic-duration" type="range" min={1} max={12} step={1} value={months} onChange={(e) => setMonths(Number(e.target.value))} className="w-full h-3 bg-slate-700 rounded-full appearance-none cursor-pointer transition-all duration-300 [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-6 [&::-webkit-slider-thumb]:h-6 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-sky-400 [&::-webkit-slider-thumb]:border-4 [&::-webkit-slider-thumb]:border-slate-900 [&::-webkit-slider-thumb]:shadow-[0_0_15px_rgba(56,189,248,0.8)] [&::-moz-range-thumb]:w-6 [&::-moz-range-thumb]:h-6 [&::-moz-range-thumb]:rounded-full [&::-moz-range-thumb]:bg-sky-400 [&::-moz-range-thumb]:border-4 [&::-moz-range-thumb]:border-slate-900" dir="rtl" aria-label="مدت زمان" aria-valuemin={1} aria-valuemax={12} aria-valuenow={months} aria-valuetext={`${months} ماه`} style={{ background: `linear-gradient(to left, #3b82f6 0%, #3b82f6 ${monthPercentage}%, #334155 ${monthPercentage}%, #334155 100%)` }} />
              <div className="flex justify-between text-xs text-slate-500 mt-2"><span>۱</span><span>۱۲</span></div>
            </div>
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  {specialProtocols.map((proto) => (
                    <button key={proto.id} type="button" aria-pressed={protocols.includes(proto.id)} onClick={() => toggleProtocol(proto.id)} className={`flex items-center justify-center gap-2 p-3 rounded-xl cursor-pointer border-2 transition-all ${protocols.includes(proto.id) ? 'bg-amber-500/20 border-amber-500/60 text-amber-300 shadow-[0_0_15px_rgba(245,158,11,0.3)]' : 'bg-slate-800/80 border-amber-500/20 text-slate-400 hover:border-amber-500/50'}`}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  {normalProtocols.map((proto) => (
                    <button key={proto.id} type="button" aria-pressed={protocols.includes(proto.id)} onClick={() => toggleProtocol(proto.id)} className={`group flex items-center justify-between gap-2 p-3 rounded-xl cursor-pointer border-2 transition-all duration-300 ${protocols.includes(proto.id) ? 'bg-pink-500/20 border-pink-500/50 text-white' : 'bg-slate-800/50 border-white/10 text-slate-400 hover:border-pink-500/30 hover:bg-slate-800'}`}>
                      <div className="flex items-center gap-2">
                        <span className={`w-5 h-5 flex items-center justify-center rounded-md border-2 transition-all duration-200 ${protocols.includes(proto.id) ? 'bg-pink-500 border-pink-500 scale-110' : 'border-slate-500 bg-transparent'}`}>
                          <Check className={`w-3.5 h-3.5 text-white transition-opacity duration-200 ${protocols.includes(proto.id) ? 'opacity-100' : 'opacity-0'}`} />
                        </span>
                        <span className="text-xs font-bold">{proto.label}</span>
                      </div>
                      <span className={`w-2 h-2 rounded-full transition-colors ${protocols.includes(proto.id) ? 'bg-pink-400 shadow-[0_0_8px_rgba(236,72,153,0.8)]' : 'bg-slate-600'}`} />
                    </button>
                  ))}
                </div>
              </div>
