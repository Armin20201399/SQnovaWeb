    ...
    <header className="sticky top-0 z-50 w-full bg-slate-950/85 backdrop-blur-xl border-b border-white/10 shadow-[0_10px_30px_rgba(0,0,0,0.3)]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 lg:h-20">
          ...
          {/* لوگو - با هاور، ستاره بزرگ و باینری کوچک می‌شود */}
          <button 
          ...
          <div className="hidden lg:flex items-center gap-3">
            <button
              onClick={() => handleNavigation('free-trial-banner')} // اصلاح شده به free-trial-banner
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-300 text-xs font-bold hover:bg-amber-500/20 hover:text-amber-200 transition-all"
            >
      ...
      >
        <div className="px-4 py-4 space-y-3 bg-slate-950/95 backdrop-blur-xl">
          ...
          <div className="pt-2 border-t border-white/10 flex flex-col gap-3">
            <button
              onClick={() => handleNavigation('free-trial-banner')} // اصلاح شده در موبایل
              className="flex items-center justify-center gap-2 w-full py-3 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-300 text-sm font-bold transition-all"
            >
              <Sparkles className="w-4 h-4" />
...
// دقیقاً همان Named Export که در App.tsx استفاده می‌شود
