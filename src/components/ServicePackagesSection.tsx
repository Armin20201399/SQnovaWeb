  ...
  return (
    ...
    >
      ...
      <div className="w-full flex flex-col items-center space-y-2">
        <span className={`text-[11px] font-bold px-3 py-1 rounded-full border ${isPopular ? 'bg-pink-500/20 text-pink-300 border-pink-500/40' : 'bg-white/5 text-slate-300 border-white/10'}`}>
        ...
        <h3 className="text-base sm:text-lg font-bold text-white group-hover:text-pink-300 transition leading-snug text-center pt-1">
          <BinaryText binaryClassName="text-white/20" leftBinary="01" rightBinary="10">{pkg.name}</BinaryText>
        </h3>
        <p className="text-xs text-slate-400 leading-relaxed text-center">{pkg.targetAudience}</p>
      </div>
      ...
      <div className="w-full pt-4 border-t border-white/10">
        <a href={pkg.telegramLink} target="_blank" rel="noreferrer" className={`w-full py-3 rounded-2xl font-bold text-xs flex items-center justify-center gap-2 transition duration-200 shadow-lg shine-effect ${isPopular ? 'bg-gradient-to-r from-purple-600 via-pink-600 to-orange-500 hover:from-purple-500 hover:to-orange-400 text-white shadow-[0_10px_25px_rgba(236,72,153,0.4)] hover:scale-105' : 'bg-white/10 hover:bg-white/20 text-white border border-white/10 hover:border-pink-400'}`}>
        ...
        {onScrollToSection && (
          <div className="p-6 rounded-3xl bg-slate-900/80 border border-amber-500/30 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="text-center sm:text-right">
              ...
              <p className="text-xs text-slate-400">با اشتراک تست می‌تونی تمام خدمات و پروتکل‌ها رو امتحان کنی، شاید آپشن مدنظرت پیدا شد!</p>
            </div>
            <button onClick={() => onScrollToSection('free-test')} className="px-6 py-3 rounded-2xl bg-amber-500/10 hover:bg-amber-400 border border-amber-500/40 hover:border-amber-300 text-amber-300 hover:text-slate-950 font-bold text-xs transition-all duration-200 flex items-center justify-center gap-2 hover:scale-105 group shine-effect flex-shrink-0">
