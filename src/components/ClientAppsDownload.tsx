      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className={`grid grid-cols-1 ${activeCategory === 'mobile' ? 'md:grid-cols-2' : 'max-w-2xl mx-auto'} gap-6 mb-12 text-center`}>
          {filteredApps.map((app) => (
            <div
              key={app.id}
              className="rounded-3xl bg-slate-900/40 border border-white/10 hover:border-pink-500/50 p-6 sm:p-8 shadow-xl transition duration-300 flex flex-col items-center justify-between space-y-6 group hover:-translate-y-1 backdrop-blur-2xl text-center shine-effect"
            >
              <div className="w-full flex flex-col items-center">
                <div className="flex items-center justify-between w-full mb-4">
                  <div className="text-right">
                    ...
                    <span className="text-[11px] text-amber-400 bg-amber-500/10 border border-amber-500/20 px-2.5 py-0.5 rounded-full font-medium mt-1 inline-block backdrop-blur-sm">
                      {app.storeBadge}
                    </span>
                  </div>
                ...
                <div className="w-full p-4 rounded-2xl bg-white/5 border border-white/5 space-y-2.5 text-xs text-slate-300 backdrop-blur-sm text-center">
                  ...
                  {app.guideSteps.map((step, sIdx) => (
                    <div key={sIdx} className="flex items-center justify-center gap-2 text-xs text-slate-300 text-center">
                      <span className="w-4 h-4 rounded-full bg-pink-500/20 text-pink-400 flex items-center justify-center flex-shrink-0 font-mono text-[10px] font-bold">
                        {sIdx + 1}
                      </span>
                      <span>{step}</span>
                    </div>
              ...
              <div className="w-full pt-2 flex items-center justify-center gap-2">
                ...
                {app.githubUrl && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="p-3.5 rounded-2xl bg-white/5 hover:bg-white/10 border border-white/10 text-slate-300 hover:text-white transition backdrop-blur-sm"
